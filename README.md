# 📝 SSJ - 内网记事本

[![Version](https://img.shields.io/badge/version-1.7.3-blue.svg)](https://github.com)
[![PHP](https://img.shields.io/badge/php-%3E%3D7.4-purple.svg)](https://php.net)
[![SQLite](https://img.shields.io/badge/database-SQLite-orange.svg)](https://sqlite.org)
[![License](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE)

一款轻量级、零依赖的个人记事本应用。基于 PHP + SQLite 构建，无需 MySQL、无需 Composer，开箱即用。适合**部署在内网服务器**，供团队多人使用。

> 🔒 **定位**：内网环境下的个人记事工具，数据隔离、部署简单、上手零门槛。

---

## ✨ 功能特性

### 核心功能
- **✏️ 笔记管理** — 新建、编辑、保存、删除笔记，支持自定义标题
- **🔍 全文搜索** — 侧边栏常驻搜索框，300ms 防抖，支持标题和内容搜索
- **🧹 回收站** — 软删除机制，笔记先移入回收站（默认保留 30 天），支持恢复和彻底删除
- **📌 笔记置顶** — 重要笔记一键置顶，置顶笔记在列表中优先显示
- **💾 定时自动保存** — 可设置 1/2/3/5/10 分钟间隔自动保存，默认每 3 分钟
- **📄 TXT 导出** — 一键导出为 UTF-8 编码的 .txt 文件

### 个性化
- **🎨 11 套护眼皮肤** — 默认白、护眼绿、暖黄纸、深色模式、牛皮纸、网格白底、网格绿底、暗夜绿、暗海蓝、暖夜色、暗网格
- **🔤 字体切换** — 默认、宋体、楷体、仿宋、Consolas、Monaco 共 6 种字体
- **📐 字号调节** — 12px ~ 24px 自由调节
- **💡 即时 Tooltip** — 工具栏按钮鼠标悬停零延迟提示

### 快捷键
- **⌨️** `Ctrl+N` 新建 | `Ctrl+F` 搜索 | `Ctrl+S` 保存 | `Esc` 清空搜索

### 安全
- **👥 多用户隔离** — 每个用户只能访问自己的笔记，数据完全隔离
- **🔐 CSRF 防护** — 所有写操作均验证 CSRF Token
- **🛡️ Cookie 安全** — HttpOnly + SameSite + Secure 属性
- **🔑 密码哈希** — 使用 `password_hash()` 安全存储密码
- **📋 登录日志** — 记录每次登录的用户名、IP、时间和状态

### 管理后台
- **👤 用户管理** — 查看所有用户列表、重置用户密码
- **⚙️ 回收站设置** — 配置回收站自动清理天数（1-365 天可调）
- **📊 统计面板** — 用户总数、笔记总数、回收站数量、今日笔记、登录统计
- **📜 登录日志** — 查看最近 50 条登录记录

### 运维
- **💿 数据库备份** — `backup.php` 脚本，支持命令行执行和定时任务
- **🪶 极致轻量** — 纯 PHP + SQLite，零 Composer 依赖，部署即用

---

## 🚀 快速开始

### 环境要求
- PHP >= 7.4
- SQLite3 扩展（PHP 默认自带）
- MBString 扩展

### 部署到服务器（3 步完成）

**第 1 步：上传文件**

将所有文件上传到 Web 服务器目录（如 `/var/www/html/SSJ/`），**注意不要上传 `data/` 目录**（数据库和日志由程序运行时自动生成）。

**第 2 步：设置权限**

```bash
chmod -R 755 data/
```

**第 3 步：修改管理员密码**

编辑 `config.php`：
```php
$config['admin_username'] = 'admin';          // 管理员用户名
$config['admin_password'] = 'your-password';  // 改成你自己的密码
```

然后浏览器访问 `http://your-server/SSJ/` 即可使用。首次访问会自动初始化数据库。

---

## 📁 项目结构

```
SSJ/
├── index.php              # 登录/注册页
├── notes.php              # 笔记主页面（编辑器 + 侧边栏）
├── api.php                # RESTful API（前后端数据交互）
├── admin.php              # 管理后台（用户管理、统计、日志）
├── admin/
│   └── changelog.php      # 更新日志展示页
├── init.php               # 初始化（数据库建表、会话、公共函数）
├── config.php             # 配置文件（版本号、数据库、管理员账号）
├── backup.php             # 数据库备份脚本（命令行/定时任务）
├── logout.php             # 退出登录
├── data/                  # 数据目录（程序自动创建）
│   ├── index.php          # 目录访问保护
│   ├── notes.db           # SQLite 数据库
│   └── app.log            # 操作日志
├── CHANGELOG.md           # 更新日志
└── README.md              # 项目说明（本文件）
```

---

## 🎨 皮肤预览

| 皮肤 | 描述 | 适用场景 |
|------|------|----------|
| 默认白 | 清爽白色背景 + 紫色主题 | 日常使用 |
| 护眼绿 | 淡绿色背景，降低屏幕刺激 | 长时间写作 |
| 暖黄纸 | 暖黄色背景，模拟纸张 | 文学创作 |
| 深色模式 | Catppuccin 风格暗色主题 | 夜间使用 |
| 牛皮纸 | 米色背景 + 横线纹理 | 复古风格 |
| 网格白底 | 浅色网格背景 | 结构化笔记 |
| 网格绿底 | 绿色网格背景 | 护眼 + 结构 |
| 暗夜绿 | 终端风格暗绿 + 微妙光晕 | 护眼编程 |
| 暗海蓝 | VS Code 风格深蓝底 | 开发环境 |
| 暖夜色 | 深暖色调，类似夜间阅读模式 | 深夜写作 |
| 暗网格 | 深紫底色 + 淡紫网格纹理 | 创意笔记 |

---

## ⬆️ 升级指南

### 升级到新版本

用新文件覆盖服务器上的旧文件即可，**注意以下规则**：

| 目录/文件 | 是否覆盖 | 说明 |
|-----------|----------|------|
| 所有 `.php` 文件 | ✅ 覆盖 | 程序文件 |
| `.md` 文档文件 | ✅ 覆盖 | 可选 |
| `data/` 目录 | ❌ 不要覆盖 | 含生产数据库和日志 |

> ⚠️ **`data/` 目录绝对不能上传覆盖**，否则会丢失所有用户笔记数据！

---

## 🖥️ 手动上传到 GitHub（无需 Git 工具）

以下教程适合不想安装 Git 工具、通过 GitHub 网页端操作的用户。

### 第 1 步：创建仓库

1. 登录 [GitHub](https://github.com)，点击右上角 **+** → **New repository**
2. 填写仓库名称（见下方推荐）
3. Description 填写：`轻量级 PHP + SQLite 内网记事本，零依赖开箱即用`
4. 选择 **Public** 或 **Private**
5. **勾选** "Add a README file"（稍后会被覆盖，没关系）
6. 点击 **Create repository**

### 第 2 步：上传文件

1. 进入刚创建的仓库页面
2. 点击 **Add file** → **Upload files**
3. 将本地项目文件**拖拽**到上传区域。**注意：不要上传以下内容**：
   - `data/` 目录（包含本地数据库和日志）
   - `.codebuddy/` 目录（IDE 数据）
4. 在 Commit message 输入：`v1.7.3 - 首版上传`
5. 点击 **Commit changes**

### 第 3 步：填写仓库信息（可选优化）

上传完成后，在仓库首页可以设置：

- **About**（右上角齿轮图标）：添加项目描述和标签 `php` `sqlite` `notes` `intranet`
- **Topics**：添加 `php`、`sqlite`、`notepad`、`web-app` 等标签，方便他人搜索

### 后续更新

每次修改代码后，同样通过 **Add file → Upload files** 上传更新的文件，GitHub 会自动追踪历史版本。

---

## 🔌 API 接口

| 方法 | Action | 说明 | CSRF |
|------|--------|------|------|
| GET | `list` | 获取笔记列表（分页） | - |
| GET | `get` | 获取单条笔记详情 | - |
| GET | `search` | 全文搜索笔记 | - |
| POST | `save` | 新建/更新笔记 | ✅ |
| POST | `delete` | 软删除（移入回收站） | ✅ |
| POST | `restore` | 恢复回收站笔记 | ✅ |
| GET | `trash` | 查看回收站列表 | - |
| POST | `permanent_delete` | 彻底删除单条笔记 | ✅ |
| POST | `emptyTrash` | 清空回收站 | ✅ |
| POST | `togglePin` | 切换笔记置顶状态 | ✅ |
| POST | `setSkin` | 设置皮肤偏好 | ✅ |
| POST | `setFont` | 设置字体/字号偏好 | ✅ |
| POST | `setAutoSave` | 设置自动保存间隔 | ✅ |

---

## 📝 更新日志

详见 [CHANGELOG.md](CHANGELOG.md)

---

## 📄 License

MIT License

---

## ⚠️ 注意事项

- **不要在生产环境开启 `display_errors`**（config.php 中已默认关闭）
- 管理员账号仅用于用户管理，**不能创建或编辑笔记**
- 删除的笔记在回收站保留 30 天后自动清理，可在管理后台调整
- 建议定期执行 `backup.php` 备份数据库
- 部署在外网时请务必配置 HTTPS
- `data/` 目录是运行时自动生成的，上传代码时不要包含此目录
